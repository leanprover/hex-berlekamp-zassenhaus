...
# `LLL_Factorization` session heap (pulls in the verified LLL basis
# reduction and Perron-Frobenius AFP entries; a heavy first build), generate the wrapper-theory
# Haskell extraction, and compile a stand-alone `lll_isabelle` driver
# binary against `scripts/oracle/bz-lll-isabelle/Main.hs`.
#
...
cache_root="${HEX_ORACLE_CACHE:-${repo_root}/.cache/oracles}/bz-lll-isabelle"
...
binary="${build_dir}/lll_isabelle"
template_dir="${repo_root}/scripts/oracle/bz-lll-isabelle"
...
acquire_lock() {
  ...
  while ! mkdir "${lock_dir}" 2>/dev/null; do
    ...
    if (( waited >= 300 )); then
      echo "setup_bz_lll_isabelle.sh: timed out waiting for ${lock_dir}" >&2
      exit 1
    fi
    sleep 1
    waited=$((waited + 1))
...
need() {
  if ! command -v "$1" >/dev/null 2>&1; then
    echo "setup_bz_lll_isabelle.sh: missing required command: $1" >&2
    exit 1
  fi
}
...
verify_archive() {
  ...
  echo "setup_bz_lll_isabelle.sh: SHA-256 mismatch for ${path}" >&2
...
if [[ ! -f "${afp_thys}/LLL_Factorization/LLL_Factorization_Impl.thy" ]]; then
  rm -rf "${afp_dir}"
  mkdir -p "${afp_dir}"
...
template_sum="$(
  shasum -a 256 \
    "${template_dir}/ROOT" \
    "${template_dir}/Hex_LLL_Factor_Export.thy" \
...
cp "${template_dir}/Hex_LLL_Factor_Export.thy" "${build_dir}/Hex_LLL_Factor_Export.thy"
...
afp_component_registered() {
  ...
  while IFS= read -r component; do
    if [[ -f "${component}/LLL_Factorization/LLL_Factorization_Impl.thy" ]]; then
      return 0
    fi
...
if [[ ! -x "${binary}" || "${build_dir}/Main.hs" -nt "${binary}" || "${code_dir}/Hex_LLL.hs" -nt "${binary}" ]]; then
